...
public class Messenger : IMessenger {
    ...
    public void Register<TMessage>(IMessageReceiver recipient, object? token, bool receiveDerivedMessagesToo, Func<TMessage, Task> action) {
        lock(registerLock) {
            ...
            lock(recipients) {
                ...
                if (!recipients.TryGetValue(messageType, out List<WeakFuncAndToken>? existingRecipient)) {
                    list = [];

                    recipients.Add(messageType, list);
                }
                else list = existingRecipient;

                WeakFunc<TMessage> weakFunc = new(recipient, action);

                WeakFuncAndToken item = new() {
                    Func = weakFunc,
                    Token = token
                };
    ...
    private static void UnregisterFromLists<TMessage>(IMessageReceiver recipient, object? token, Func<TMessage, Task>? action, Dictionary<Type, List<WeakFuncAndToken>> list) {
        ...
        if (list.Count == 0 || !list.TryGetValue(messageType, out List<WeakFuncAndToken>? existingList)) return;

        lock(list) {
            foreach(WeakFuncAndToken item in existingList) {
